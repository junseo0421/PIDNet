...
class PIDNet(nn.Module):
    ...
    def forward(self, x):
        ...
        temp_s = self.dfm(x_, x, x_d)
        x_ = self.final_layer(temp_s)

        if self.augment: 
            x_extra_p = self.seghead_p(temp_p)
            x_extra_d = self.seghead_d(temp_d)
            return [x_extra_p, x_, x_extra_d], [temp_p, temp_s, temp_d]
        else:
            return x_      

# def get_seg_model(cfg, imgnet_pretrained):
def get_seg_model(num_classes, load_path, name):
    ...
    if load_path and os.path.isfile(load_path):
        ckpt = torch.load(load_path, map_location='cpu')  # 필요 시 weights_only=False 옵션 추가 가능
        ...
        for k, v in state.items():
            k2 = k
            if k2.startswith('module.'): k2 = k2[len('module.'):]
            if k2.startswith('model.'):  k2 = k2[len('model.'):]
            if k2 in model_dict and model_dict[k2].shape == v.shape:
                filtered[k2] = v

        msg = f'Loaded {len(filtered)} params from {os.path.basename(load_path)}'
        logging.info(msg)
        model_dict.update(filtered)
        model.load_state_dict(model_dict, strict=False)
    else:
        logging.info('No pretrained weights loaded.')
